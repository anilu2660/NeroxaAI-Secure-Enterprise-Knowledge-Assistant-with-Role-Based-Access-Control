...
function normalizeExecution(value: unknown): AssistantQueryResponse | null {
  ...
  return {
    ...
    route: (data.route as AssistantQueryResponse["route"]) ?? "casual",
    routeConfidence: typeof data.route_confidence === "number" ? data.route_confidence : null,
    rewrittenQuery: typeof data.rewritten_query === "string" ? data.rewritten_query : null,
    cached: Boolean(data.cached),
    model: typeof data.model === "string" ? data.model : null,
    chunksRetrieved: typeof data.chunks_retrieved === "number" ? data.chunks_retrieved : 0,
    ...
    toolStatus:
      data.tool_status === "success" || data.tool_status === "not_executed"
        ? data.tool_status
        : null,
    toolResult: data.tool_result ?? null,
    agentPlan:
      data.agent_plan && typeof data.agent_plan === "object"
        ? (data.agent_plan as AssistantQueryResponse["agentPlan"])
        : null,
    agentSteps: Array.isArray(data.agent_steps)
      ? (data.agent_steps as AssistantQueryResponse["agentSteps"])
      : [],
    messageId: typeof data.message_id === "string" ? data.message_id : null,
    createdAt: typeof data.created_at === "string" ? data.created_at : null,
    webSearchStatus:
      typeof data.web_search_status === "string" ? data.web_search_status : null,
  };
}

function executionToCitations(execution: AssistantQueryResponse | null): Citation[] {
  return (execution?.sources ?? []).map((src, index) => ({
    id: src.id || `cit_${index}`,
    documentId: src.documentId || `doc_${index}`,
